...
class BaseRepository {
  ...
  add(item) {
    return this.dbClient.then((db) =>
      db.collection(this.collection).insertOne(item)
    );
  }

  addMany(items) {
    return this.dbClient.then((db) =>
      db.collection(this.collection).insertMany(items)
    );
  }
  ...
  async bulkWrite(operations) {
    ...
    return await db.collection(this.collection).bulkWrite(operations);
  }
  ...
  list() {
    return this.dbClient.then((db) => db.collection(this.collection).find());
  }

  listFiltered(filter) {
    return this.dbClient.then((db) => {
      const data = db.collection(this.collection).find(filter.query || {});
